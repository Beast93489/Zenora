...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  final List<Map<String, dynamic>> _avatars = [
    ...
    {'emoji': '🎵', 'title': 'Music Soul',   'color': Color(0xFF1DB954)},
    {'emoji': '🎨', 'title': 'Artist',       'color': Color(0xFFFF69B4)},
    {'emoji': '⚡', 'title': 'Vibe Checker', 'color': Color(0xFFFFB800)},
    {'emoji': '🌙', 'title': 'Night Owl',    'color': Color(0xFF4A90D9)},
    {'emoji': '🔥', 'title': 'Streak King',  'color': Color(0xFFFF6B35)},
    {'emoji': '💜', 'title': 'Empath',       'color': Color(0xFF9B59B6)},
    {'emoji': '🤖', 'title': 'AI Lover',     'color': Color(0xFF00B4B4)},
    {'emoji': '🌊', 'title': 'Chill One',    'color': Color(0xFF007A7A)},
  ];
  ...
  Future<void> _loadData() async {
    ...
    final emotionEmoji = {'joy':'😊','sadness':'😔','anger':'😤','fear':'😰','surprise':'😲','neutral':'😐','hype':'🔥','romantic':'🥰'};
    final emotionColor = {'joy':Color(0xFFFFB800),'sadness':Color(0xFF4A90D9),'anger':Color(0xFFE74C3C),'fear':Color(0xFF9B59B6),'surprise':Color(0xFF00B4B4),'neutral':Color(0xFF95A5A6),'hype':Color(0xFFFF6B35),'romantic':Color(0xFFFF69B4)};
    ...
    for (int i = 6; i >= 0; i--) {
      ...
      final dayEntries = weekEntries.where((e) {
        ...
        try { final dt = (ts as dynamic).toDate() as DateTime; return dt.day == day.day && dt.month == day.month; } catch (_) { return false; }
      }).toList();
      final emotion = dayEntries.isNotEmpty ? (dayEntries.first['emotion'] as String? ?? 'neutral') : '';
      weekMoods.add({'day': dayLabels[day.weekday - 1], 'emoji': emotion.isNotEmpty ? (emotionEmoji[emotion] ?? '😐') : '', 'color': emotion.isNotEmpty ? (emotionColor[emotion] ?? Color(0xFF95A5A6)) : Color(0x1AFFFFFF), 'hasEntry': emotion.isNotEmpty});
    }

    final emotionCounts = <String, int>{};
    for (final e in weekEntries) { final em = e['emotion'] as String? ?? 'neutral'; emotionCounts[em] = (emotionCounts[em] ?? 0) + 1; }
    String dominant = 'Chill';
    if (emotionCounts.isNotEmpty) {
      final top = emotionCounts.entries.reduce((a, b) => a.value > b.value ? a : b).key;
      const labels = {'joy':'Joyful','sadness':'Sad','anger':'Angry','fear':'Anxious','surprise':'Surprised','neutral':'Chill','hype':'Hyped','romantic':'Romantic'};
      dominant = labels[top] ?? 'Chill';
    }

    final earnedBadges = List<String>.from(profile?['badges'] ?? []);
    final updatedBadges = _allBadges.map((b) => {...b, 'earned': earnedBadges.contains(b['id'])}).toList();
    ...
    Future.delayed(const Duration(milliseconds: 300), () { if (mounted) _badgeController.forward(); });
  }
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Text('🎭  Pick Your Avatar', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    Text(av['title'] as String, style: TextStyle(color: isSelected ? color : Colors.white38, fontSize: 10, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal), textAlign: TextAlign.center),
                    if (isSelected) ...[const SizedBox(height: 3), Container(width: 14, height: 2, decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(1)))],
                  ]),
                ),
              );
            }).toList(),
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            _gradBtn('Save Changes 💾', () async {
              ...
              updates['username'] = userCtrl.text.trim();
              updates['university'] = uniCtrl.text.trim();
              if (tempAvatar.isNotEmpty) updates['avatar'] = tempAvatar;
  ...
  void _openNotifications() {
    showModalBottomSheet(context: context, backgroundColor: Colors.transparent,
      builder: (_) => StatefulBuilder(builder: (ctx, setLocal) => _sheet(title: '🔔  Notifications', child: Column(mainAxisSize: MainAxisSize.min, children: [
        _toggleTile('Daily Check-in Reminder', 'so we don\'t have to @ you 😅', _notifDaily, (v) { setState(() => _notifDaily = v); setLocal(() {}); }),
        _divider(),
      ...
      builder: (_) => DraggableScrollableSheet(initialChildSize: 0.7, maxChildSize: 0.95, minChildSize: 0.4,
        builder: (_, ctrl) => Container(
          ...
          child: Column(children: [
            ...
            const SizedBox(height: 16),
            Expanded(child: ListView.separated(controller: ctrl, padding: const EdgeInsets.symmetric(horizontal: 20), itemCount: faqs.length, separatorBuilder: (_, __) => _divider(), itemBuilder: (_, i) => _faqTile(faqs[i]['q']!, faqs[i]['a']!))),
            Padding(padding: const EdgeInsets.all(20), child: _gradBtn('Contact Support 📧', () { Navigator.pop(context); _snack('Email: ds7794092@gmail.com 📧'); })),
          ]),
        ),
      ));
  }
  ...
  void _openAbout() {
    showModalBottomSheet(context: context, backgroundColor: Colors.transparent, isScrollControlled: true,
      builder: (_) => DraggableScrollableSheet(initialChildSize: 0.85, maxChildSize: 0.95, minChildSize: 0.5,
        builder: (_, ctrl) => Container(
          decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF1A0533), Color(0xFF0F0F1E), Color(0xFF003333)]), borderRadius: const BorderRadius.vertical(top: Radius.circular(28)), border: Border.all(color: const Color(0x33FFFFFF))),
          child: SingleChildScrollView(controller: ctrl, padding: const EdgeInsets.all(24), child: Column(children: [
            Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(2))),
            const SizedBox(height: 24),
            TweenAnimationBuilder<double>(tween: Tween(begin: 0.0, end: 1.0), duration: const Duration(milliseconds: 800), curve: Curves.elasticOut,
              builder: (_, val, __) => Transform.scale(scale: val, child: Container(width: 88, height: 88,
                decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFF5C2D91), Color(0xFF007A7A)]), boxShadow: [BoxShadow(color: const Color(0xFF5C2D91).withValues(alpha: 0.6), blurRadius: 24, spreadRadius: 4)]),
                child: const Center(child: Text('◉', style: TextStyle(color: Colors.white, fontSize: 40)))))),
            const SizedBox(height: 16),
            ShaderMask(shaderCallback: (bounds) => const LinearGradient(colors: [Color(0xFF9B59F5), Color(0xFF00B4B4)]).createShader(bounds),
              child: const Text('ZENORA', style: TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.w900, letterSpacing: 6))),
            const SizedBox(height: 4),
            const Text('Know Your Mind', style: TextStyle(color: Color(0xFF00B4B4), fontSize: 14, fontStyle: FontStyle.italic, letterSpacing: 2)),
            const SizedBox(height: 8),
            Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 5), decoration: BoxDecoration(color: const Color(0x1AFFFFFF), borderRadius: BorderRadius.circular(12)), child: const Text('v1.0.0 Beta • Research Edition', style: TextStyle(color: Colors.white38, fontSize: 11))),
            const SizedBox(height: 28),
            Container(width: double.infinity, padding: const EdgeInsets.all(20),
              decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [const Color(0xFF5C2D91).withValues(alpha: 0.3), const Color(0xFF007A7A).withValues(alpha: 0.2)]), borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFF9B59F5).withValues(alpha: 0.4))),
              child: Column(children: [
                Row(children: [
                  Container(width: 56, height: 56, decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFF5C2D91), Color(0xFF007A7A)]), boxShadow: [BoxShadow(color: const Color(0xFF5C2D91).withValues(alpha: 0.5), blurRadius: 12)]), child: const Center(child: Text('D', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)))),
                  const SizedBox(width: 14),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('Divyanshu Sharma', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.bold)),
                    const Text('@Beast93489', style: TextStyle(color: Color(0xFF9B59F5), fontSize: 12)),
                    const SizedBox(height: 4),
                    Row(children: [_techChip('Dev 👨‍💻', const Color(0xFF9B59F5)), const SizedBox(width: 6), _techChip('Research 🔬', const Color(0xFF00B4B4))]),
                  ])),
                ]),
                const SizedBox(height: 14),
                Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0x0DFFFFFF), borderRadius: BorderRadius.circular(12)),
                  child: const Text('B.Tech CSE • Chandigarh University • Class of 2027\nBuilding Zenora as research project targeting IEEE Access', style: TextStyle(color: Colors.white54, fontSize: 12, height: 1.5), textAlign: TextAlign.center)),
              ])),
            const SizedBox(height: 20),
            const Align(alignment: Alignment.centerLeft, child: Text('🛠️ Tech Stack', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.bold))),
            const SizedBox(height: 12),
            Wrap(spacing: 8, runSpacing: 8, children: [
              ...
              _techChip('Speech-to-Text', const Color(0xFF00B4B4)),
              _techChip('Google Sign-In', const Color(0xFF4285F4)),
            ]),
            const SizedBox(height: 20),
            _infoTile('🎯', 'Mission', 'Help students understand emotional wellbeing through AI-powered multi-modal mood tracking.'),
            _divider(),
            _infoTile('🔬', 'Research Target', 'IEEE Access / JMIR Mental Health. Multi-modal emotion detection using ML.'),
            _divider(),
            _infoTile('💌', 'Contact', 'ds7794092@gmail.com\ngithub.com/Beast93489/Zenora'),
            const SizedBox(height: 24),
  ...
  // ── Reusable widgets ─────────────────────────────────────────
  Widget _techChip(String label, Color color) => Container(
  ...
  Widget _sheet({required String title, required Widget child}) => Container(
    decoration: BoxDecoration(color: const Color(0xFF1A0533), borderRadius: const BorderRadius.vertical(top: Radius.circular(28)), border: Border.all(color: const Color(0x33FFFFFF))),
    padding: const EdgeInsets.fromLTRB(24, 16, 24, 24),
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(2))),
      if (title.isNotEmpty) ...[const SizedBox(height: 16), Text(title, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold))],
  ...
  Widget _gradBtn(String label, VoidCallback onTap) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF5C2D91), Color(0xFF007A7A)]), borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: const Color(0xFF5C2D91).withValues(alpha: 0.4), blurRadius: 12, offset: const Offset(0, 4))]),
      child: Center(child: Text(label, style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.bold)))));

  Widget _divider() => Divider(color: Colors.white.withValues(alpha: 0.07), height: 1);

  Widget _toggleTile(String title, String subtitle, bool value, Function(bool) onChanged) => ListTile(
    contentPadding: EdgeInsets.zero,
    title: Text(title, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w500)),
    subtitle: Text(subtitle, style: const TextStyle(color: Colors.white38, fontSize: 12)),
    trailing: Switch(value: value, onChanged: onChanged, activeColor: const Color(0xFF9B59F5)));

  Widget _infoTile(String emoji, String title, String body) => Padding(
  ...
  Widget _faqTile(String q, String a) => ExpansionTile(
    tilePadding: EdgeInsets.zero, iconColor: const Color(0xFF9B59F5), collapsedIconColor: Colors.white38,
    title: Text(q, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w500)),
    children: [Padding(padding: const EdgeInsets.only(bottom: 12), child: Text(a, style: const TextStyle(color: Colors.white54, fontSize: 12, height: 1.5)))]);
  ...
  Widget _editField(String label, TextEditingController ctrl, IconData icon) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    TextField(controller: ctrl, style: const TextStyle(color: Colors.white),
      decoration: InputDecoration(prefixIcon: Icon(icon, color: Colors.white38, size: 20), filled: true, fillColor: const Color(0x1AFFFFFF),
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide.none),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: Color(0xFF9B59F5), width: 1.5)))),
  ]);

  Widget _themeOption(BuildContext ctx, StateSetter setLocal, ZenoraTheme theme, String label, String subtitle, List<Color> colors) {
    final selected = themeNotifier.theme == theme;
    return GestureDetector(
      onTap: () { HapticFeedback.selectionClick(); themeNotifier.setTheme(theme); setLocal(() {}); setState(() {}); Navigator.pop(ctx); _snack('$label activated! 🎨'); },
      child: AnimatedContainer(duration: const Duration(milliseconds: 200), padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(color: selected ? const Color(0x1A9B59F5) : const Color(0x0DFFFFFF), borderRadius: BorderRadius.circular(16), border: Border.all(color: selected ? const Color(0xFF9B59F5) : const Color(0x1AFFFFFF), width: selected ? 2 : 1)),
        child: Row(children: [
          ClipRRect(borderRadius: BorderRadius.circular(10), child: SizedBox(width: 48, height: 36, child: Row(children: colors.map((c) => Expanded(child: Container(color: c))).toList()))),
          const SizedBox(width: 16),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)), Text(subtitle, style: const TextStyle(color: Colors.white38, fontSize: 12))])),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: AnimatedContainer(
        ...
        decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: themeNotifier.bgGradient)),
        child: SafeArea(
          child: _loading
              ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [CircularProgressIndicator(color: Color(0xFF9B59F5)), SizedBox(height: 16), Text('loading your era... 💫', style: TextStyle(color: Colors.white38, fontSize: 13))]))
  ...
  Widget _buildHeader() {
    return Container(
      width: double.infinity, padding: const EdgeInsets.fromLTRB(20, 16, 20, 24),
      child: Column(children: [
        Row(children: [
          ...
          const Spacer(),
          GestureDetector(onTap: _openEditProfile, child: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: const Color(0x1AFFFFFF), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.edit_outlined, color: Colors.white, size: 18))),
        ]),
        const SizedBox(height: 28),
        AnimatedBuilder(
          animation: _avatarController,
          builder: (_, __) => GestureDetector(
            onTap: () => _openAvatarPicker(_selectedAvatar, (picked) async {
              setState(() => _selectedAvatar = picked);
              await FirebaseService.updateUserProfile({'avatar': picked});
              _snack('avatar updated! slay 🎭');
            }),
            child: Stack(alignment: Alignment.bottomRight, children: [
              Transform.scale(scale: _avatarScale.value, child: Container(
                width: 92, height: 92,
                decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFF5C2D91), Color(0xFF007A7A)]), boxShadow: [BoxShadow(color: const Color(0xFF5C2D91).withValues(alpha: 0.5), blurRadius: _avatarGlow.value, spreadRadius: 2)]),
                child: Center(child: _selectedAvatar.isNotEmpty
                    ? Text(_selectedAvatar, style: const TextStyle(fontSize: 44))
                    : Text(_name.isNotEmpty ? _name[0].toUpperCase() : 'Z', style: const TextStyle(color: Colors.white, fontSize: 38, fontWeight: FontWeight.bold))),
              )),
              Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: const Color(0xFF9B59F5), shape: BoxShape.circle, border: Border.all(color: const Color(0xFF0F0F1E), width: 2)), child: const Icon(Icons.edit, color: Colors.white, size: 12)),
            ]),
          ),
        ...
        const SizedBox(height: 6),
        const Text('tap to change avatar', style: TextStyle(color: Colors.white24, fontSize: 10)),
        const SizedBox(height: 12),
        ...
        Container(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [const Text('✨', style: TextStyle(fontSize: 14)), const SizedBox(width: 6), Text('Mostly $_dominantMood this week', style: const TextStyle(color: Colors.white70, fontSize: 13))]),
        ),
      ]),
    );
  }
  ...
  Widget _buildStatsRow() => Padding(
    ...
    child: Row(children: [
      _statCard('⚡', '$_zenoPoints', 'Zeno Pts', const Color(0xFF9B59F5)),
      const SizedBox(width: 10),
      _statCard('🔥', '$_streak', 'Streak', const Color(0xFFFF6B35)),
      const SizedBox(width: 10),
      _statCard('📝', '$_totalEntries', 'Entries', const Color(0xFF00B4B4)),
      const SizedBox(width: 10),
      _statCard('🏅', '${_earnedBadges.length}', 'Badges', const Color(0xFFFFB800)),
    ]));
  ...
  Widget _statCard(String emoji, String value, String label, Color color) => Expanded(
    child: Container(
      ...
      child: Column(children: [Text(emoji, style: const TextStyle(fontSize: 20)), const SizedBox(height: 4), Text(value, style: TextStyle(color: color, fontSize: 18, fontWeight: FontWeight.bold)), Text(label, style: const TextStyle(color: Colors.white38, fontSize: 9), textAlign: TextAlign.center)])));

  Widget _buildWeekMoods() => Padding(
    ...
    child: Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [Text('📅', style: TextStyle(fontSize: 18)), SizedBox(width: 8), Text('This Week', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))]),
        const SizedBox(height: 4),
        const Text('your 7-day mood map', style: TextStyle(color: Colors.white38, fontSize: 11)),
        const SizedBox(height: 16),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: _weekMoods.map((m) {
          final hasEntry = m['hasEntry'] as bool;
          return Column(children: [
            AnimatedContainer(duration: const Duration(milliseconds: 300), width: 40, height: 40,
              decoration: BoxDecoration(color: hasEntry ? (m['color'] as Color).withValues(alpha: 0.15) : const Color(0x0AFFFFFF), borderRadius: BorderRadius.circular(12), border: Border.all(color: hasEntry ? (m['color'] as Color).withValues(alpha: 0.5) : const Color(0x1AFFFFFF))),
              child: Center(child: Text(hasEntry ? (m['emoji'] as String) : '·', style: TextStyle(fontSize: hasEntry ? 20 : 16, color: Colors.white24)))),
            const SizedBox(height: 6),
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Text('(unlock them all fr)', style: TextStyle(color: Colors.white38, fontSize: 11)),
          const Spacer(),
          Text('$earned / ${_allBadges.length}', style: const TextStyle(color: Color(0xFF9B59F5), fontSize: 13, fontWeight: FontWeight.bold)),
        ]),
        const SizedBox(height: 14),
        GridView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return AnimatedBuilder(
              ...
              builder: (_, __) {
                final delay = i * 0.1;
                final anim = CurvedAnimation(parent: _badgeController, curve: Interval(delay.clamp(0.0, 0.9), (delay + 0.3).clamp(0.1, 1.0), curve: Curves.elasticOut));
                return Transform.scale(scale: anim.value, child: GestureDetector(
                  onTap: () { HapticFeedback.selectionClick(); _snack(isEarned ? '${badge['icon']} ${badge['title']}: ${badge['genZ']} ✅' : '${badge['title']}: ${badge['desc']} — not yet!'); },
                  child: Container(
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(color: isEarned ? color.withValues(alpha: 0.15) : const Color(0x0DFFFFFF), borderRadius: BorderRadius.circular(16), border: Border.all(color: isEarned ? color.withValues(alpha: 0.5) : const Color(0x1AFFFFFF), width: isEarned ? 1.5 : 1), boxShadow: isEarned ? [BoxShadow(color: color.withValues(alpha: 0.2), blurRadius: 8)] : null),
                    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Text(badge['icon'] as String, style: TextStyle(fontSize: 28, color: isEarned ? null : Colors.white.withValues(alpha: 0.2))),
                      const SizedBox(height: 6),
                      Text(badge['title'] as String, style: TextStyle(color: isEarned ? Colors.white : Colors.white24, fontSize: 11, fontWeight: FontWeight.bold), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis),
                      const SizedBox(height: 2),
                      Text(isEarned ? (badge['genZ'] as String) : (badge['desc'] as String), style: TextStyle(color: isEarned ? color.withValues(alpha: 0.8) : Colors.white.withValues(alpha: 0.15), fontSize: 9), textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
                      if (isEarned) ...[const SizedBox(height: 4), Container(width: 16, height: 2, decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(1)))],
                    ]),
                  )));
              });
          }),
  ...
  Widget _buildSettingsList() => Padding(
    padding: const EdgeInsets.all(20),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        decoration: BoxDecoration(color: const Color(0x0DFFFFFF), borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0x1AFFFFFF))),
        child: Column(children: List.generate(_settings.length, (i) {
          final s = _settings[i];
