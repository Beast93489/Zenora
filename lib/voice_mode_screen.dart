...
class _VoiceModeScreenState extends State<VoiceModeScreen>
  ...
  void initState() {
    ...
    _rippleController = AnimationController(
      ...
      ..repeat();
    _ripple1 = Tween<double>(begin: 0.8, end: 1.6).animate(
        CurvedAnimation(parent: _rippleController, curve: Curves.easeOut));
    _ripple2 = Tween<double>(begin: 0.8, end: 1.6).animate(
        CurvedAnimation(
            parent: _rippleController,
            curve: const Interval(0.3, 1.0, curve: Curves.easeOut)));
    _ripple3 = Tween<double>(begin: 0.8, end: 1.6).animate(
        CurvedAnimation(
            parent: _rippleController,
    ...
    if (_isListening) {
      setState(() {
        for (int i = 0; i < _waveHeights.length; i++) {
          ...
          _waveHeights[i] = base + _random.nextDouble() * 0.5 * (_soundLevel / 100 + 0.3);
          _waveHeights[i] = _waveHeights[i].clamp(0.1, 1.0);
        }
      });
    } else {
      for (int i = 0; i < _waveHeights.length; i++) {
  ...
  Future<void> _startListening() async {
    if (!_speechAvailable) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('mic permission nahi hai yaar! 😅',
    ...
    await _speech.listen(
      ...
      onSoundLevelChange: (level) {
        setState(() => _soundLevel = (level + 2).clamp(0.0, 10.0) * 10);
      },
  ...
  Future<void> _analyzeVoice() async {
    ...
    try {
          ...
          'contents': [
            {
              'parts': [
                {
                  'text':
                      'Analyze the emotion in this voice transcript from a mental wellness app.\n\n'
      ...
      setState(() {
        _detectedEmotion = emotion;
        _aiResponse = _getFallback(emotion);
        _isAnalyzing = false;
      });
      _resultController.forward();
    }
  ...
  String _getFallback(String emotion) {
    final responses = {
      ...
      'sadness': 'Thank you for speaking your truth. 💙 It takes courage to voice what hurts. Be gentle with yourself today.',
  ...
  Color get _activeColor {
    if (_detectedEmotion.isNotEmpty) {
      return (_emotionData[_detectedEmotion]?['color'] as Color?) ??
      ...
      body: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [
              _activeColor.withValues(alpha: _isListening ? 0.4 : 0.2),
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(children: [
                      ...
                      children: [
                        ...
                        if (_isListening)
                          AnimatedBuilder(
                            ...
                            builder: (_, __) => Container(
                              width: 200 * _ripple3.value,
                              height: 200 * _ripple3.value,
                              decoration: BoxDecoration(
                                shape: BoxShape.circle,
                                border: Border.all(
                                  color: _activeColor.withValues(
                                      alpha: (1 - _rippleController.value * 0.8)
                                          .clamp(0.0, 0.3)),
                                  width: 1,
                                ),
                              ),
                        ...
                        if (_isListening)
                          AnimatedBuilder(
                            animation: _ripple2,
                            builder: (_, __) => Container(
                              width: 160 * _ripple2.value,
                              height: 160 * _ripple2.value,
                              decoration: BoxDecoration(
                                shape: BoxShape.circle,
                                border: Border.all(
                                  color: _activeColor.withValues(
                                      alpha: (1 - _rippleController.value * 0.7)
                                          .clamp(0.0, 0.4)),
                                  width: 1.5,
                                ),
                              ),
                        ...
                        if (_isListening)
                          AnimatedBuilder(
                            animation: _ripple1,
                            builder: (_, __) => Container(
                              width: 120 * _ripple1.value,
                              height: 120 * _ripple1.value,
                              decoration: BoxDecoration(
                                shape: BoxShape.circle,
                                color: _activeColor.withValues(
                                    alpha: (1 - _rippleController.value * 0.9)
                                        .clamp(0.0, 0.15)),
                              ),
                            ),
                          ),
                  ...
                  Text(
                    _isListening
                        ? 'tap red button to stop ⏹️'
                        : _isAnalyzing
                            ? 'AI padh raha hai tera dard... 🤖'
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(children: [
                            ...
                            Text(
                              _isListening ? 'Live Transcript' : 'You said',
                              style: const TextStyle(
                        ...
                        child: Column(children: [
                          ...
                          Row(children: [
                            Container(
                              ...
                              child: const Text('⚡ +15 Zeno Points!',
                                  style: TextStyle(
                                      color: Color(0xFF9B59F5),
                                      fontWeight: FontWeight.bold,
                                      fontSize: 13)),
